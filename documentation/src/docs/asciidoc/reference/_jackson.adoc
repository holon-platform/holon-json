...
With a properly configured `ObjectMapper` instance, a `PropertyBox` can be serialized and deserialized just like any another object. As described in the <<PropertyBox>> section, you need to declare the `PropertySet` to use as a _Context_ resource at `PropertyBox` deserialization time.
