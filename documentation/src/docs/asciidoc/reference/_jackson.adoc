...
The `JacksonAutoConfiguration` Spring Boot _auto-configuration_ class is provided to automatically configure a `ObjectMapper` for `PropertyBox` JSON serialization/deserialization. An `ObjectMapper` instance must be available as _bean_ in the Spring context. 
